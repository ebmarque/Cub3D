...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/05/28 21:32:23 by tiago             #+#    #+#             */
/*   Updated: 2024/05/28 21:36:13 by tiago            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_texture	*get_text_info(t_ray *ray)
{
	...
	text = NULL;
	if (cubed()->content->map[(int)ray->pos.y][(int)ray->pos.x] == 'C')
		text = cubed()->door;
	else if (cubed()->content->map[(int)ray->pos.y][(int)ray->pos.x] < 10)
	{
		if (!ray->side)
		{
			if (ray->dir.x < 0)
